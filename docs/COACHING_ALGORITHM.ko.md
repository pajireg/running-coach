...
`안전성은 코드가 담당합니다. llm_driven 모드에서는 LLM이 hard bound 안에서 코칭 판단을 담당합니다.`
...
### 개인화 pace-zone engine

워크아웃 페이스 target은 더 이상 고정 상수가 아닙니다. planner는 주간 skeleton마다 `PaceZoneEngine` 결과를 만들고, 이 target을 결정론적 workout step에 사용합니다. `llm_driven` 모드에서는 같은 pace-zone 체계를 prompt에 전달하고 `PaceZoneIntegrity` 로 강제합니다.
...
Garmin에는 여전히 단일 속도 강제가 아니라 pace zone으로 전달됩니다. target pace는 zone의 중심값이고, `WorkoutManager`가 세션별 margin을 적용합니다.
