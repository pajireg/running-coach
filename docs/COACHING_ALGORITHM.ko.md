...
> - `legacy` (default, free tier) — 알고리즘이 7일 skeleton 을 만들고 결정론적 템플릿이 workout step 과 한국어 설명을 채움.
...
Running Coach는 LLM이 처음부터 끝까지 계획을 만들어내는 시스템이 아닙니다. 현재 코치 엔진은 선수 히스토리 저장, 규칙 기반 안전 제어, 결정론적 legacy 계획, 선택적 LLM-driven 계획을 결합한 구조입니다.
...
3. **규칙 기반 계획 생성** (`legacy`) 또는 **context 조립 + 안전 검증** (`llm_driven`)
   Legacy는 회복, 부하, 실행 이력, 제약을 바탕으로 7일 skeleton을 생성합니다.
   LLM-driven은 `CoachingContext` 를 조립하고 LLM이 계획을 만들며, `SafetyValidator` 가 위반을 자동 보정합니다.
4. **워크아웃 조립과 검증**
   Legacy는 결정론적 step 템플릿과 한국어 설명 템플릿으로 skeleton을 채웁니다.
   LLM-driven은 LLM 출력을 파싱하고 15개 safety rule로 검증 및 보정합니다.
...
완료 활동의 거리와 시간은 Garmin activity summary를 기준으로 사용합니다. Garmin
calendar activity row는 필드별 단위가 일정하지 않을 수 있으므로 코칭 판단의 canonical
입력으로 쓰지 않습니다.
...
7. 결정론적 legacy 템플릿으로 workout step과 설명을 만듭니다.
8. 계획과 의사결정 근거를 저장합니다.
9. Garmin 워크아웃과 Google Calendar를 동기화합니다.

`auto` 서비스 모드에서는 활성 계획이 충분히 최신이면 6-9단계를 건너뜁니다. 계획 범위가 비어 있거나, 마지막 계획 이후 새 Garmin 활동이 생겼거나, 회복 지표가 크게 나빠졌거나, 핵심 세션을 수행하지 않았거나, base 볼륨 누락이 누적되었거나, 여러 계획 워크아웃을 수행하지 못한 경우 재계획합니다. recovery run 1회 미수행은 추가 휴식으로 보고 그 자체만으로는 재계획하지 않습니다.
...
### 개인화 pace capability profile

워크아웃 페이스 target은 더 이상 고정 상수가 아닙니다. `legacy` 모드에서는 planner가 `PaceZoneEngine`의 center pace를 사용해 결정론적 workout step을 만듭니다. `llm_driven` 모드에서는 prompt에 threshold 근거, reference center pace, safety band로 구성된 pace capability profile을 전달하고, LLM이 band 안에서 구체 target pace를 선택합니다.

우선순위:

- Garmin 젖산역치 페이스
- 설정된 레이스 목표 페이스
- 5K, 10K, 하프, 풀마라톤 PR에서 추정한 threshold 페이스
- 페이스 데이터가 없을 때의 보수적 기본 threshold 페이스

엔진은 다음 reference center pace와 safety band를 계산합니다.
...
Garmin에는 여전히 단일 속도 강제가 아니라 pace zone으로 전달됩니다. `llm_driven` 모드에서는 LLM이 선택한 target pace를 중심으로 사용하고, `PaceBandIntegrity`는 safety band 밖의 값만 경계값으로 보정합니다. 이후 `WorkoutManager`가 세션별 margin을 적용합니다.
...
- 정리는 저장된 Garmin schedule ID를 먼저 쓰고, 이후 workout template ID를 사용
...
- long run은 7일 계획 범위 안에 사용자 선호 long run 가용 날짜가 있으면 반드시
  그 날짜 중 하나에 배치하고, 없을 때만 가능한 주말을 우선합니다.
- quality는 주중과 사용자 선호 요일을 우선합니다.
- 불가능한 요일은 무조건 휴식일로 고정합니다.
- 비러닝 부하가 높으면 러닝 일수와 품질훈련 수를 줄입니다.

즉 안전 경계는 코드가 먼저 고정합니다. 다만 장거리 최대 1회, hard 최대 2회,
최소 휴식 1일, ACWR 1.5 같은 값은 보편 진리가 아니라 현재 기본 정책입니다.
이 값들은 `CoachingContext.plan_policy`에 담겨 프롬프트와 SafetyValidator가 함께
사용하며, 사용자별 값은 `user_preferences.coaching_policy`에서 공급할 수 있습니다.

`llm_driven`에서는 프롬프트에 요일 숫자만 넘기지 않고 `availability_rules`에서 계산한
구체적인 `longRunAllowedDates`를 함께 전달합니다. 이 목록이 비어 있지 않으면
...
LLM full-plan 출력 스키마는 `weekly`가 아니라 `horizon` 근거 블록과
`sevenDayVolumeTargetKm`를 사용합니다. 7일 계획은 고정된 캘린더 한 주의 이야기가 아니라 rolling horizon입니다. 서비스는
매일 체크인하며 Garmin 수행 기록, 회복 지표, 피드백을 다시 읽고 오늘부터 다음 7일을
일관되게 유지합니다. workout description은 각 세션이 현재 7일 흐름 안에서 맡는
역할을 기준으로 작성합니다. 설명이 오래된 캘린더 주간 프레이밍으로 흐르면
deterministic rolling-horizon 설명으로 교체합니다.
...
- 세션 타입 변경
- 위험한 볼륨 급증 생성
- 사용자가 가능한 날이고 심각한 활성 부상이 없는데 긴 연속 휴식만 배치
...
- 잘못된 pace 형식 제거
- 0초 step 보정
- Garmin에 이상하게 보이지 않도록 steady step은 분 단위, 반복 step은 30초 단위로
  duration 정규화
- 잘못된 날짜나 이름 수정
