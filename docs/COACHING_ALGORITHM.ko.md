...
> - `legacy` (default, free tier) — 알고리즘이 주간 skeleton 을 만들고 결정론적 템플릿이 workout step 과 한국어 설명을 채움.
...
3. **규칙 기반 계획 생성** (`legacy`) 또는 **context 조립 + 안전 검증** (`llm_driven`)
   Legacy는 회복, 부하, 실행 이력, 제약을 바탕으로 주간 skeleton을 생성합니다.
...
7일 계획은 고정된 캘린더 한 주의 이야기가 아니라 rolling horizon입니다. 서비스는
