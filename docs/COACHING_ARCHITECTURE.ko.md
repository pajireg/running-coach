...
9. `legacy`: 규칙 기반 skeleton 을 만들고 LLM 이 그 안에 step 을 채웁니다.
...
- `src/running_coach/coaching/context.py` — `CoachingContextBuilder` 가 점수·pace zones·14일 실행 이력·부상·피드백·가용성·훈련 배경을 `CoachingContext` dataclass 로 조립
...
- `src/running_coach/clients/gemini/planner.py` — 레거시 skeleton 기반 planner (fallback 용으로 보존)
...
- `safety/`: 하드 제약(계획 시작일=오늘, long_run 주 1회 상한, 연속 quality 금지, 부상 차단, ACWR 상한, 최소 1일 휴식, pace zone 정합, 워크아웃 이름 표준화 등) 자동 보정
- `planners/llm_driven.py`: `CoachingContext → prompt → Gemini → Pydantic → SafetyValidator` 파이프라인; 파싱/쿼터/미수렴 실패 시 legacy fallback
- `clients/gemini/planner.py`: 기존 skeleton 구성 경로; `COACH_PLANNER_MODE=legacy` 에서 활성
...
- LT / PR / race target 기반 PaceZone 계산
...
- `SafetyValidator` 로 강제되는 안전 룰:
  ...
  - step pace 는 반드시 PaceZone 값 중 하나
...
LLM 출력이 안전 룰을 위반하면 validator 가 자동 보정하고 로그를 남깁니다. 보정이 수렴하지 못하면 `legacy` 로 fallback 합니다.

### `legacy` 모드 (fallback, `llm_driven` 검증 완료 전까지 기본)

Legacy 경로는 `planner._build_weekly_skeleton` 에서 규칙 기반 skeleton 을 만들고 LLM 에게 step 세부만 채우게 합니다.
...
- Garmin 업로드 전 `PaceZoneEngine`으로 워크아웃 페이스 target을 개인화합니다.
