...
def _ctx_with_chronic_daily(chronic_daily_km: float) -> CoachingContext:
    return CoachingContext(
        ...
        scores=CoachingScores(
            ...
            chronic_ewma_load=chronic_daily_km,
        ),
...
class TestAcwrCap:
    def test_passes_when_no_chronic_baseline(self):
        ctx = _ctx_with_chronic_daily(0.0)
    ...
    def test_passes_when_ratio_below_cap(self):
        # 기본 plan 총 ~43km. chronic 6km/day → weekly 42km, ratio ≈ 1.02.
        ctx = _ctx_with_chronic_daily(6.0)
        plan = make_plan(["base", "quality", "base", "base", "base", "long_run", "rest"])
        assert AcwrCap().check(plan, ctx) == []

    def test_detects_high_ratio(self):
        # chronic 2km/day → weekly 14km. 43/14 ≈ 3.1 > 1.5
        ctx = _ctx_with_chronic_daily(2.0)
        plan = make_plan(["base", "quality", "base", "base", "base", "long_run", "rest"])
    ...
    def test_corrector_preserves_long_run_and_quality(self):
        """long_run / quality 는 구조적 축소에서 보존."""
        rule = AcwrCap()
        ctx = _ctx_with_chronic_daily(2.0)
        plan = make_plan(["base", "quality", "base", "base", "base", "long_run", "rest"])
        violations = rule.check(plan, ctx)
    ...
    def test_corrector_structurally_reduces_for_extreme_ratio(self):
        ...
        rule = AcwrCap()
        ctx = _ctx_with_chronic_daily(0.5)  # weekly 3.5, ratio >10
        plan = make_plan(["base", "quality", "base", "base", "base", "long_run", "rest"])
    ...
    def test_describe_mentions_weekly_target_km(self):
        ctx = _ctx_with_chronic_daily(5.0)  # weekly 35
        desc = AcwrCap().describe(ctx)
        assert "km" in desc
        # cap = 35 * 1.5 = 52km
        assert "52" in desc
