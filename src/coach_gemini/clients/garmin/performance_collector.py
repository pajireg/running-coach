...
class PerformanceDataCollector:
    ...
    def collect(self, target_date: DateType) -> PerformanceMetrics:
        """퍼포먼스 지표 수집 (통합)
        ...
        try:
            ...
            for group in all_recs:
                ...
                for pr in recs:
                    ...
                    # 값 추출
                    val = (pr.get('value') or pr.get('recordValue') or
                           pr.get('prValue') or pr.get('personalRecordValue') or pr.get('time'))
            ...
            pr_summary = ", ".join([pr.display for pr in prs]) if prs else f"{len(prs_raw) if prs_raw else 0} 개의 기록"
