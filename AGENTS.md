...
- **Multi-tenant first.** `users.external_key` identifies each user. Any new query, service, or background job MUST accept a `user_id` rather than a singleton. Do not introduce new singleton state keyed on a single deployment credential.
...
- **Per-user configuration belongs in the database, not env vars.** `Settings` is process-global. Any new knob that a user should customize (schedule times, include_strength, coach_planner_mode, preferred LLM, max weekly km, notification preferences) should read from `user_preferences`, falling back to `Settings` defaults. Do not add new `COACH_*` env vars for behavior that should eventually be per-user.
- **Secrets are per-user and encrypted at rest.** Credentials and OAuth tokens are stored in `user_integration_credentials` with app-level encryption keyed by `user_id`. Never design a feature that assumes one shared `.env` credential. Local single-tenant deployments seed credentials from env at startup via `_seed_local_runtime_credentials`.
- **Scheduler must not assume a fixed routine.** Do not hardcode `05:00,17:00` or `Asia/Seoul`. Respect per-user schedule and timezone values from `user_preferences`.
...
- **Data portability & deletion.** Any new table must be listed in the export flow and deletable by user id. Design schemas with `ON DELETE CASCADE` from `users`.
...
- **Observability.** Keep metric labels bounded (`rule_id`, `severity`, `mode`). Do not add unbounded labels like `user_id`, `date`, `activity_id` to counters; use structured logs or a per-user dashboard query instead.
